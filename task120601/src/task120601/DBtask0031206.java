...
public class DBtask0031206 {
	...
	// MySQL 연결 정보
	static final String DB_URL = "jdbc:mysql://localhost/causwtask1206?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC"; // 데이터베이스
																																											// 주소
	...
	private static void queryReferenceBooks(Connection conn, Scanner scanner) throws SQLException {
		...
		String sql = "SELECT r.ISBN, r.title, r.author, r.publisher, r.year, i.name as instructor_name "
				+ "FROM reference_book r " + "JOIN refers ref ON r.ISBN = ref.ISBN "
	...
	private static void borrowBook(Connection conn, Scanner scanner) throws SQLException {
		...
		// 수강생 확인
		String checkEnrollmentSql = "SELECT t.ID FROM takes t " + "JOIN refers r ON t.course_id = r.course_id "
				+ "AND t.sec_id = r.sec_id " + "AND t.semester = r.semester " + "AND t.year = r.year "
